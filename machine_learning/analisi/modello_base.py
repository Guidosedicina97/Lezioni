...
class ModelloBase (ABC):
    ...
    @staticmethod
    def analisi_valori_univoci(df,variabili_dra_droppare=None):
        print("********** VALORI UNIVOCI DATAFRAME **********")
        if variabili_dra_droppare:
            df= df.drop(variabili_dra_droppare,axis=1)
        for col in df.columns:
            print(f"In colonna {col} abbiamo {df[col].nunique()} valori univoci")
            for value in df[col].unique():
                print(value)

    # metodo per analisi_dataset indici statistici
    @staticmethod
